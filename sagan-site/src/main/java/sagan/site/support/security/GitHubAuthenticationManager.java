...
public class GitHubAuthenticationManager implements AuthenticationManager {
    ...
    public GitHubAuthenticationManager(ClientRegistrationRepository clients, OAuth2UserService<OAuth2UserRequest, OAuth2User> githubMembers) {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        OAuth2AccessToken oauthToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, token, Instant.now(), Instant.now().plus(Duration.ofHours(1)));
